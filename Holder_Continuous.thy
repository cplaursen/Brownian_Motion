...
proof
  assume *: ?L
  {
    ...
    then obtain \<epsilon> C where "\<epsilon> > 0" "C \<ge> 0"
      ...
      using * unfolding local_holder_on_def apply auto
    ...
    have "\<exists>\<epsilon> > 0. \<exists>C \<ge> 0. \<forall>r\<in>cball t \<epsilon> \<inter> D. \<forall>s\<in>cball t \<epsilon> \<inter> D. dist (\<phi> r) (\<phi> s) \<le> C * dist r s powr \<gamma>"
      apply (rule_tac exI [where x = "\<epsilon>/2"])
      apply (auto simp: \<open>\<epsilon> > 0\<close>)
...
proof
  assume *: "local_holder_on 1 D \<phi>"
  {
    ...
    then obtain \<epsilon> C where eC: "\<epsilon> > 0" "C \<ge> 0"
      ...
      using * powr_to_1 unfolding local_holder_on_cball apply auto
...
proof -
  {
    ...
    {
      fix s r assume *: "s \<in> D" "r \<in> D"
      then have "dist s t \<le> ?e \<and> dist r t \<le> ?e \<longrightarrow> dist (\<phi> r) (\<phi> s) \<le> C * dist r s powr g"
        using eC by simp
      moreover have "dist s t \<le> ?e \<and> dist r t \<le> ?e \<longrightarrow> dist r s \<le> 1"
        by (smt (verit) dist_triangle2 half_bounded_equal)
      ultimately have "dist s t \<le> ?e \<and> dist r t \<le> ?e \<longrightarrow> dist (\<phi> r) (\<phi> s) \<le> C * dist r s powr h"
        using powr_mono_le1[of g h "dist r s", OF assms(2-4)] eC(2) mult_left_mono by fastforce
    }
...
proof safe
  ...
  from assms obtain C where C: "C \<ge> 1" "(\<forall>r\<in>X. \<forall>s\<in>X. dist (\<phi> r) (\<phi> s) \<le> C * dist r s powr \<gamma>)"
    ...
    by (metis linorder_not_le mult_1 mult_less_cancel_right1 order_le_less order_less_le_trans powr_ge_pzero)      
  {
...
proof -
  ...
  text \<open> \<rho> is the Lebesgue number of the cover \<close>
